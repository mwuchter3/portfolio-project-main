...
- **4/9 @ 1:50 pm est**: <!-- TODO: fill out with due date and time (e.g., 10/17 @ 3:10 PM EST) and delete this comment -->
...
I am going to represent a matrix using an array of arrays of ints because this basically is a matrix. The representation will be restriced to only having ints inside of the arrays, and the arrays on the inside of the first array all need to be of the same size. The array will need to be interpreted as the first element in the first array being the first row of the matrix, and the first element in the first array being the first element in the first row of the matrix, and so on.
